...
![](tetracyclines-images/Tetracycline_MOA.png){fig-align="center" width="650"}
...
| Drug         | Half-life   | V~D~ (L/kg) | Protein Binding | Lipophilicity |
|--------------|-------------|-------------|-----------------|---------------|
| Tetracycline | 7 hours     | \~1.5       | 24-65%          | Baseline      |
| Doxycycline  | 12-16 hours | \~0.7       | 82-93%          | 5× baseline   |
| Minocycline  | 16-21 hours | 0.14-0.7    | 70-80%          | 10× baseline  |
| Tigecycline  | 37-67 hours | 7-10        | 71-89%          | Very high     |
...
::: notes
Complex medication schedules reduce compliance. Sometimes switching to azithromycin or a fluoroquinolone is more practical[@Moffa2024].
:::
...
::: notes
Efflux is the most common mechanism. Ribosomal protection is especially important in gram-positive organisms [@Chopra2001; @Roberts2005].
:::
...
::: notes
Tet(X) enzymes are concerning because they can inactivate even the newer glycylcyclines. Surveillance is ongoing [@Thaker2010].
:::
...
::: aside
The glycylcycline structure was specifically designed to evade common resistance mechanisms. However, Tet(X) can still inactivate some of these [@Bergeron2016].
:::
...
::: aside
The mortality signal led to FDA warnings. Tigecycline should not be used for VAP or bacteremia due to low serum levels. Mortality difference was not seen with the higher dose [@BabinchakTige2005; @FDAWarningTige2010].
:::
...
::: notes
The large volume of distribution means excellent tissue levels but inadequate serum concentrations for bacteremia [@BabinchakTige2005; @FDAWarningTige2010].
:::
...
::: notes
Eravacycline was developed to improve on tigecycline's limitations. It's currently limited to cIAI [@Solomkin2014; @MolloyErava2018].
:::
...
::: aside
The oral formulation is a major advantage. Loading dose of 450 mg day 1, then 300 mg daily [@OKeefeCABP2019].
:::
...
::: aside
The ability to complete therapy orally makes omadacycline practical for outpatient treatment or early discharge [@Stets2019].
:::
...
::: aside
Sarecycline represents a deliberate narrow-spectrum approach to minimize collateral damage and resistance selection [@Moffa2024].
:::
...
::: notes
Selection depends on indication, need for oral therapy, and patient-specific factors [@Moffa2024].
:::
...
::: notes
Chloramphenicol's toxicity limited its use in developed countries, but it remains important in resource-limited settings [@Mandell2024].
:::
...
::: notes
Oral chloramphenicol is still available for travelers to areas with limited alternatives [@Mandell2024].
:::
...
::: notes
The risk of aplastic anemia, though rare, is the main reason chloramphenicol fell from widespread use in developed countries [@Mandell2024].
:::
...
digraph decision_tree {
    rankdir=TB
    node [shape=diamond, style=filled, fillcolor="#E8F4FD", fontname="Helvetica"]
    edge [fontname="Helvetica", fontsize=10]

    pregnant [label="Is the patient\npregnant or <8 years old?"]
    renal [label="Is renal function\nimpaired?"]

    node [shape=box, style="filled,rounded", fillcolor="#FDECEA"]
    avoid [label="Avoid tetracyclines\n(except life-threatening RMSF)"]

    node [shape=box, style="filled,rounded", fillcolor="#E8F5E9"]
    doxy_renal [label="Use DOXYCYCLINE\n(no renal adjustment needed)"]

    node [shape=box, style="filled,rounded", fillcolor="#FFF8E1"]
    atypical [label="Atypicals / Tick-borne:\nDoxycycline"]
    cns [label="CNS infection:\nMinocycline"]
    acne [label="Acne:\nDoxycycline, Minocycline,\nor Sarecycline"]
    mdr [label="MDR GN / GN infections:\nTigecycline or Eravacycline"]

    pregnant -> avoid [label="Yes"]
    pregnant -> renal [label="No"]
    renal -> doxy_renal [label="Yes"]
    renal -> atypical [label="No"]
    renal -> cns [style=invis]
    renal -> acne [style=invis]
    renal -> mdr [style=invis]

    node [shape=plaintext, style="", fillcolor=none]
    choose [label="Choose based on indication"]
    renal -> choose [label="No"]
    choose -> atypical
    choose -> cns
    choose -> acne
    choose -> mdr

    {rank=same; atypical; cns; acne; mdr}
}
```
...
<center>S = susceptible, V = variable, R = resistant</center>
...
<center>*Values in μg/mL; breakpoints per CLSI*</center>
...
<center>\*After 100 mg loading dose; \*\*After 300 mg × 2 loading</center>
